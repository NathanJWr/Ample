...
static inline struct Array_Buffer *ARR_BASE_POINTER(void *arr_ptr) {
  return (
      (struct Array_Buffer *)((char *)arr_ptr - sizeof(struct Array_Buffer)));
}

#define ARRAY_ALLOCATION_SIZE(arr_ptr, size)                                   \
  (sizeof(struct Array_Buffer) + sizeof(*arr_ptr) * size)
...
#define ARRAY_PUSH(arr_ptr, item)                                              \
  if (arr_ptr == NULL) {                                                       \
    struct Array_Buffer *buff =                                                \
        calloc(1, ARRAY_ALLOCATION_SIZE(arr_ptr, ARR_DEFAULT_INIT_SIZE));      \
    buff->count++;                                                             \
