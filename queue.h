...
#define QUEUE_RESIZE(queue_p, new_capacity) do { \
    /* this is probably super expensive, but it keeps the start of the array memory usable */ \
    if ((queue_p)->head > 0) { \
        /* move the array so it starts at index 0 */ \
        size_t size = (queue_p)->size; \
        /* using a for loop here because you can't memcpy regions that may overlap */ \
    ...
    void* mem = realloc ((queue_p)->mem, new_capacity * sizeof(*(queue_p)->mem)); \
    assert (mem); \
...
#define QUEUE_FRONT(queue_p) \
    (queue_p)->mem[(queue_p)->head]

#define QUEUE_EMPTY(queue_p) \
    ((queue_p)->size == 0)
